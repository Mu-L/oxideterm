...
const ACP_ADAPTER_ARG: &str = "--acp-adapter";
...
pub fn run_from_env_if_requested() {
    ...
    // The adapter branch must return before Tauri initializes so stdout stays a
    // clean ACP transport and no window, logging layer, or app state is created.
    let config = AdapterConfig::from_args(args).unwrap_or_else(|error| {
        eprintln!("{error}");
        std::process::exit(2);
    });
    ...
    let exit_code = match runtime.block_on(run_adapter(config)) {
        Ok(()) => 0,
        Err(error) => {
...
async fn run_adapter(config: AdapterConfig) -> agent_client_protocol::Result<()> {
...
impl AdapterConfig {
    fn from_args(args: impl IntoIterator<Item = OsString>) -> Result<Self, &'static str> {
        let mut args = args.into_iter();
        let provider_arg = args
            .next()
            .and_then(|value| value.into_string().ok())
            .ok_or("usage: oxideterm --acp-adapter <claude-code|codex> [--command <command>] [--arg <arg>...]")?;
        let provider = AdapterProvider::parse(&provider_arg)?;
        let mut command = None;
        let mut extra_args = Vec::new();
        while let Some(arg) = args.next().and_then(|value| value.into_string().ok()) {
            match arg.as_str() {
                "--command" => {
                    command = args.next().and_then(|value| value.into_string().ok());
                    if command.is_none() {
                        return Err("--command requires a value");
                    }
                }
                "--arg" => {
                    let value = args
        ...
        Ok(Self {
            provider,
            command: command.unwrap_or_else(|| provider.default_command().to_string()),
            extra_args,
        })
    }
}
...
impl AdapterProvider {
    fn parse(value: &str) -> Result<Self, &'static str> {
        match value {
            ...
            "codex" => Ok(Self::Codex),
            _ => Err("unknown ACP adapter provider"),
        }
    }
