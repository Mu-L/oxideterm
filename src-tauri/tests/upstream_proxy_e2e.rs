...
use oxideterm_lib::ssh::{
    AuthMethod, ProxyChain, ProxyHop, SshClient, SshConfig, UpstreamProxyAuth, UpstreamProxyConfig,
    UpstreamProxyProtocol, accept_host_key, connect_via_proxy_for_test, upstream_proxy_from_env,
};
use tokio::{
...
async fn local_sshd_upstream_proxy_e2e_matrix() {
    ...
    // The proxy-chain test helper trusts unknown host keys for the session, but
    // it does not accept an explicit expected fingerprint. Seed the in-memory
    // host-key cache so this E2E never depends on the user's known_hosts file.
    accept_host_key("127.0.0.1", sshd.port, &sshd.host_fingerprint).expect("seed host key cache");

    connect_direct(target_config(&sshd).with_upstream_proxy(socks.config())).await;
    connect_direct(target_config(&sshd).with_upstream_proxy(socks_password.config_with_password()))
        .await;
    connect_proxy_jump(&sshd, socks.config()).await;
    connect_direct(target_config(&sshd).with_upstream_proxy(http.config())).await;
    connect_proxy_jump(&sshd, http.config()).await;
    ...
    connect_direct(target_config(&sshd)).await;
    assert_eq!(
        socks.accept_count(),
        direct_count,
...
async fn connect_direct(config: SshConfig) {
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        SshClient::new(config).connect(None),
    )
    .await
    .expect("SSH connection timed out");
    let _session = result.expect("SSH connection failed");
}

async fn connect_proxy_jump(sshd: &SshdFixture, upstream_proxy: UpstreamProxyConfig) {
    let chain = ProxyChain::new()
        .add_hop(sshd.proxy_hop())
        .with_upstream_proxy(Some(upstream_proxy));
    let auth = AuthMethod::key(sshd.client_key.display().to_string(), None);
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        connect_via_proxy_for_test(&chain, "127.0.0.1", sshd.port, &sshd.username, &auth, 10),
    )
    .await
    .expect("ProxyJump connection timed out");
    let _connection = result.expect("ProxyJump connection failed");
}
...
impl SshdFixture {
    fn start() -> Self {
        ...
        let home_guard = EnvVarGuard::set("HOME", dir.display().to_string());
...
impl EnvVarGuard {
    fn set(key: &'static str, value: String) -> Self {
        ...
        // The ignored E2E test is run serially and needs a private HOME so
        // host-key verification cannot read or write the user's known_hosts.
        unsafe {
...
fn unique_temp_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    env::temp_dir().join(format!("oxideterm-tauri-ssh-e2e-{nanos}"))
}
